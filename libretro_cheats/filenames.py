...
**Extension-preserving.** A cheat file is a `.cht` and RetroArch finds
it by that extension; a truncation that ate it would leave a file nothing
loads.

What differs from the cores version is what the input looks like. A
buildbot core filename is already close to bare. These are No-Intro game
names and carry every piece of punctuation that implies --
`Legend of Zelda, The - A Link to the Past (USA).cht`,
`Pocky & Rocky (USA).cht`, `Doom 64 (USA) (Rev A).cht` -- most of which
...
def safe_filename(raw: str, fallback: str = "cheat.cht") -> str:
